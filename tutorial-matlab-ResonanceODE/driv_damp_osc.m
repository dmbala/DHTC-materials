...
function dd_oscillator(fnumber)
    ...
    b = 0.3;        % drag coeficient s
    m = 1.0;        % mass 
    x0 = 1.0;       % initial position
    v0 = 1.0;       % initial velocitie
    F0 = 1.0;       % strength of applied frequency 
    tBegin = 0;     % time begin
    tEnd = 80;      % time end

    % Applied frequency is chosen from random generator 
    rng('shuffle');          % seeds the random number generator based on the current time. Random numbers are not repeated.
    omega_app = rand(1)*2.5; % driving frequency selected from random number generator 
